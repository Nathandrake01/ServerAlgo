...
def env_overrides(broker, creds, port, ws_port, preserve=None):
    ...
    ov = {
        ...
        "TZ": "Asia/Kolkata",
        "HOST_SERVER": f"http://127.0.0.1:{port}",    # login scripts + engine read this
        "NGROK_ALLOW": "FALSE",
    }
    if broker == "kotak":
        ov.update({
            "KOTAK_MOBILE": creds.get("KOTAK_MOBILE", ""),
            "KOTAK_MPIN": creds.get("KOTAK_MPIN", ""),
            "KOTAK_TOTP_SECRET": creds.get("KOTAK_TOTP_SECRET", ""),
        })
    else:
        ov.update({
            "ZERODHA_USER_ID": creds.get("ZERODHA_USER_ID", ""),
...
# Applied ON THE SERVER against a .env seeded from .sample.env: replace managed
# keys in place, append any that the sample didn't have (KOTAK_*, HOST_SERVER…).
_ENV_PATCH_PY = '''
import json, sys
env_path, ov_path = sys.argv[1], sys.argv[2]
...
for ln in open(env_path, encoding="utf-8").read().splitlines():
    ...
    if s and not s.startswith("#") and "=" in s:
        k = s.split("=", 1)[0].strip()
        if k in ov:
            out.append(f"{k}='{ov[k]}'"); seen.add(k); continue
    out.append(ln)
for k, v in ov.items():
    if k not in seen:
        out.append(f"{k}='{v}'")
open(env_path, "w", encoding="utf-8").write("\\n".join(out) + "\\n")
print("applied", len(ov), "overrides,", len(ov) - len(seen), "appended")
'''


def _emit_yaml(obj, indent=0):
    """Tiny YAML serializer for our config dicts — avoids requiring PyYAML on the
    user's machine (a common install failure on Macs). Handles nested dicts, lists,
    and scalars. All strings are single-quoted so values like '09:18' stay strings."""
    pad = "  " * indent
    lines = []
...
def _scalar(v):
    ...
    if isinstance(v, (int, float)):
        return repr(v)
    if isinstance(v, dict):      # empty dict
        return "{}"
    s = str(v).replace("'", "''")
    return f"'{s}'"


def generate_config(strategy, multiplier, port):
    """Generate the CLIENT config — only the knobs the client controls.

    All proprietary parameters (entry/exit times, trigger %, zone thresholds,
    SL/TP, hedging, routing) live compiled inside the engine (locked_params) and
    are NOT written to any readable file. The engine overlays quantity_lots / mode
    / openalgo from this config onto its locked defaults."""
    if strategy not in ("pr_0918", "pr_0946", "gamma", "delta"):
        raise ValueError(f"unknown strategy {strategy}")
    cfg = {
        "strategy": {"name": strategy, "mode": "live", "quantity_lots": multiplier},
        "openalgo": {"host": f"http://127.0.0.1:{port}", "api_key": "auto"},
    }
    return _emit_yaml(cfg) + "\n"


# Per-strategy engine class. PR sleeves use the default engine (no override);
# gamma/delta pass their own engine_cls into the shared runner.
_ENGINE_CLS = {
    "gamma": ("gamma_engine", "LongGammaRescue"),
    "delta": ("delta_engine", "DeltaShortStrangle"),
}


def entry_script(strategy, config_filename):
    """Generate the thin entry-point script.

    The engine is a package (engine/*.so) whose modules use relative imports, so
    it must be imported as `engine.pr_runner`, with the directory that CONTAINS
    `engine/` on sys.path. run() resolves the config filename against
    engine/../configs (i.e. strategies/configs/), so pass a bare filename.
    """
    if strategy in _ENGINE_CLS:
        mod, cls = _ENGINE_CLS[strategy]
        engine_import = f"from engine.{mod} import {cls}\n"
        run_call = f'run("{config_filename}", engine_cls={cls})'
    else:
        engine_import = ""
        run_call = f'run("{config_filename}")'
    return f'''"""{strategy} - auto-generated entry point (do not edit)."""
import sys
from pathlib import Path

# Directory containing the engine/ package (i.e. .../strategies/).
...
# Provisioning script executed INSIDE the OpenAlgo container. It boots the app
# context (which loads .env), ensures the admin user exists, then generates and
# stores an API key using OpenAlgo's own helpers. Prints "APIKEY=<hex>" on success.
_PROVISION_PY = '''
import sys, secrets
sys.path.insert(0, "/app")   # running a script file sets sys.path[0] to its own dir
...
with app.app_context():
    ...
    # The OpenAlgo login also needs a TOTP; surface the user's secret so the
    # deployer can put it in .env for hands-free auto-login.
    try:
